...
data = '/Users/dominikjohn/Library/Mobile Documents/com~apple~CloudDocs/Documents/1_Projects/FourRod_SBXIData/'
os.chdir(data)

num_masks = 13
gamma = 500 # Value of perspex at 25 keV
wavelength = 4.9594*10**-5 # [microns]
prop = 0.7*10**6 # [microns]
pixel_size = 6.5 # [microns]
savedir = '/Users/dominikjohn/Library/Mobile Documents/com~apple~CloudDocs/Documents/1_Projects/FourRod_SBXIData/'
crop = np.s_[:,140:2360]
ff = np.double(np.asarray(Image.open('Flatfield.tif')))[crop] # Cropping required to get just illuminated field-of-view
rows, columns = ff.shape
import os
flatfield_path = 'Flatfield.tif'
if os.path.exists(flatfield_path):
    print(f"Flatfield file exists at {flatfield_path}")
else:
    print(f"Flatfield file not found at {flatfield_path}")

print(f"Shape of ff: {ff.shape}")
...
for k in range(1,int(num_masks+1)):
        ...
        ir = np.double(np.asarray(Image.open('ReferenceSpeckle_{}.tif'.format(str(i)))))[crop]
        isa = np.double(np.asarray(Image.open('SamplePlusSpeckle_{}.tif'.format(str(i)))))[crop]
        print(f"Shape of Is: {Is.shape}")
        print(f"Shape of Is[0, :, :]: {Is[0, :, :].shape}")
        ir = (ir-dc)/(ff-dc)
        isa = (isa-dc)/(ff-dc)
