...
def gammacal(filepath, gamma_guess, sod, odd, wavelength, det_voxel):
    ...
    mu_recon_um = mu_recon
    mask = ~np.isnan(mu_recon_um)
    position_microns = position_microns[mask]
    mu_recon_um = mu_recon_um[mask]
...
filepath = '/data/mct/22203/results/P6_Manual/recon_T_pvc_modified/Values5.csv'
gammaguess = 147
sod = 15*10**6 # source-to-object distance, microns
odd = .15*10**6 # object-to-detector distance, microns
wavelength = 5*10**-5 # wavelength, microns
detector_size = 1.444 # detector pixel size, microns
