...
loader = DataLoader(Path("/data/mct/22203/"), "P6_ReverseOrder")
...
# calibration = .8567 # I think this one is for p6 manual
calibration = 739
...
mu_volume = mu_volume[2:-3, ...]
n1_volume = inverse[0, 0] * edensity_volume + inverse[0, 1] * mu_volume
n2_volume = inverse[1, 0] * edensity_volume + inverse[1, 1] * mu_volume

imshow(n1_volume.transpose(1, 0, 2)[280], title=material1['name'] + ' (v/v)')
imshow(n2_volume.transpose(1, 0, 2)[280], title=material2['name'] + ' (v/v)')
...
material2_name = material2['name'].lower()
plt.savefig(os.path.join(home_dir, f'p6_reverse_mat_decomp_{material2_name}_4binned.png'), dpi=1200, bbox_inches='tight')
plt.show()
...
home_dir = os.path.expanduser('~')
material1_name = material1['name'].lower()
plt.savefig(os.path.join(home_dir, f'p6_reverse_mat_decomp_{material1_name}_4binned.png'), dpi=1200, bbox_inches='tight')
plt.show()

fig, ax = plot_slice(edensity_volume.transpose(1, 0, 2),
                     slice_idx=300,
                     pixel_size=psize,
                     vmin=200,
                     vmax=650,
                     title='Electron density [1/nm$^3$]',
                     cmap='grey',
                     fontsize=24,
                     show_size=True,
                     )
home_dir = os.path.expanduser('~')
plt.savefig(os.path.join(home_dir, f'p6_reverse_mat_edensity_4binned.png'), dpi=1200, bbox_inches='tight')
plt.show()

fig, ax = plot_slice(mu_volume.transpose(1, 0, 2),
                     slice_idx=300,
                     pixel_size=psize,
                     vmin=0,
                     vmax=4.5,
                     title='Attenuation coefficient [1/cm]',
                     cmap='grey',
                     fontsize=24,
                     show_size=True,
                     )
home_dir = os.path.expanduser('~')
plt.savefig(os.path.join(home_dir, f'p6_reverse_mat_attenuation_4binned.png'), dpi=1200, bbox_inches='tight')
plt.show()
