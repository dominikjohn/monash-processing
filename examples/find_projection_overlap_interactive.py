...
def interactive_projection_alignment(data_loader, angle_index1: int, angle_index2: int):
    ...
    """
    # Load projections
    proj1 = data_loader.load_processed_projection(angle_index2, 'dx')
    proj2 = data_loader.load_processed_projection(angle_index1, 'dx')
    ...
    ax1.imshow(proj1, cmap='gray', vmin=-0.6, vmax=0.2)
    ax1.set_title('Projection 1')

    ax2.imshow(proj2_flipped, cmap='gray', vmin=-0.6, vmax=0.2)
    ax2.set_title('Projection 2 (Flipped)')

    # Create slider
    slider = Slider(slider_ax, 'Horizontal Shift', 1325, 1335, valinit=initial_shift, valstep=1)
...
def update_alignment(shift):
    ...
    # Update plots
    ax3.imshow(proj1_aligned, cmap='gray', vmin=-0.6, vmax=0.6)
    ax3.set_title('Projection 1 (Aligned)')

    im4 = ax4.imshow(composite, cmap='gray', vmin=-0.6, vmax=0.6)
